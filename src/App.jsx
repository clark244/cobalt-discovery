...
// Optional browser speech features — feature-detected, both toggle-gated and off by default.
...
export default function App() {
  ...
  const [speakAloud, setSpeakAloud] = useState(false); // TTS: read the guide's turns aloud
  const [listening, setListening] = useState(false);   // STT: mic actively capturing
  const recognitionRef = useRef(null);

  // Speak a guide turn aloud via the browser's built-in speech synthesis.
  const speak = (t) => {
    if (!TTS_OK || !t) return;
    try {
      window.speechSynthesis.cancel();
      window.speechSynthesis.speak(new SpeechSynthesisUtterance(t));
    } catch (e) { /* never block on TTS */ }
  };
  const toggleSpeak = () => {
    setSpeakAloud((on) => {
      if (on && TTS_OK) window.speechSynthesis.cancel(); // turning off: stop mid-sentence
      return !on;
    });
  };
          ...
          <div className="flex items-center gap-3">
            {TTS_OK && (
              <button
                ...
                style={{ color: speakAloud ? COBALT : "#9CA3AF" }}
                title={speakAloud ? "Stop reading the guide's replies aloud" : "Read the guide's replies aloud"}
